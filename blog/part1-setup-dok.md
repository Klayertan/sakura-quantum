...
> 実測メモ：`nproc` と `free` で見ると、v100-32gb のタスクでは 4 vCPU / 54GB、h100-80gb では 20 vCPU / 216GB が見えました（表はカタログ値）。

### 高火力 VRT との違い：どっちを借りる？

さくらで H100 を借りる方法は DOK だけではありません。**高火力 VRT** は H100 付きの仮想マシン（VM）を借りるサービスで、SSH でログインして普通のサーバーとして使えます。

| | 高火力 VRT（VM 型） | 高火力 DOK（コンテナ型） |
|---|---|---|
| 借りるもの | H100 付きの仮想サーバー 1 台 | コンテナ 1 回分の実行 |
| 使い方 | SSH でログインして対話的に作業 | Docker イメージを渡すと実行して終了 |
| 課金 | サーバーが存在・稼働している間（放置中も）【TODO: 最新の料金を公式ページで確認】 | タスク実行中の秒数だけ（H100 は ¥0.28/秒、最低 60 秒） |
| 環境 | 自分でセットアップ、状態が残る | 毎回イメージから起動、状態は残らない |
| 向いている作業 | 長時間の学習、Jupyter での試行錯誤、常駐サービス | ベンチマーク、パラメータを変えた一括実験、定期実行 |
| 注意点 | 止め忘れると使っていない時間も課金 | ビルド〜実行〜ログ確認のサイクルが必要 |

今回のベンチマークは「同じスクリプトを流して結果ファイルを受け取る」だけなので DOK を選びました。実際、H100 での smoke テストは 6 分半・約 110 円で済み、終わった瞬間に課金も止まります。Docker イメージがそのまま実験の記録になり、読者も同じイメージで再現できるのも利点です。

逆に、コードを書き換えながら何度も GPU で試したい段階なら、VRT で VM を立てて SSH で作業する方が速いでしょう。**「作る」は VRT、「回す」は DOK**、という使い分けがしっくりきます。
