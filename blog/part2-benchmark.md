...
料金は ¥0.28/秒 × 実行時間で計算した目安です。DOK の「請求情報」で見ると、**この記事の 2 回と、第 1 回の smoke テスト 2 回を合わせた H100 の利用が 38 分 8 秒で ¥643**（V100 での失敗分は 2 分 45 秒で ¥3）。全額が新規利用者向けのトライアル割引でまかなわれ、支払いは 0 円でした。
